...
| **本文件** | 想直接上手技能的人 | 8 个技能**分别是什么、什么时候用、怎么接龙** |
...
8 个技能不是一堆孤立工具，它们对应两条完整的论文生命周期。先定位自己在写哪种论文，再决定从哪个技能入手。
...
### rebuttal-drafter — 审稿回复的逐点装订员

- **定位**：基于 handbook 3.5 的 Paper Revision LaTeX 模板，把 Reviewer 意见翻译成一套可投递的结构化回复：`meta.tex` + `r1.tex` / `r2.tex` / `r3.tex`，正文修改用 `\revision{blue}` 标蓝，回复表格按"comment 编号 / 修改内容 / 修改章节"逐行对齐。
- **什么时候触发**：
  - 投稿后拿到审稿意见，要写 rebuttal / reviewer response
  - NeurIPS / ICLR / ACL 等会议的 discussion period 追问，需要追加一轮回复
  - AC 要求 meta-response 或总体 summary
  - 收到 major revision 意见，要把论文正文与回复装订成同一个 Revision 包
- **关键纪律**：每条 reviewer comment 必须逐点对应（不合并不省略）；论文正文修改必须用 `\revision{blue}` 标蓝，并在回复里引用对应章节编号；每份 `rX.tex` 顶部必须有一张汇总表（原 comment 编号 / 修改内容 / 修改章节）；meta.tex 负责贯穿多位 reviewer 的公共关切，避免重复粘贴。
- **产出**：完整的 `response/` 目录（meta + r1/r2/r3）+ main.tex 接入片段（`\input{response/r1}` 等）+ `\revision{}` / `\marginpar{}` commands.tex 补丁 + 可提交的总览说明信。
- **对应 handbook 章节**：[3.5 写作细节与 Checklist](../../../handbook/03_Paper_Writing/3.5_写作细节与Checklist.md)（Paper Revision LaTeX 模板与 Response to Reviewer Comments 段落）
- **上下游**：上游是 **pre-submission-reviewer**（审稿人视角的自查往往会提前发现 rebuttal 里要辩护的点）；下游是正文改稿循环，回到 **tech-paper-template** 或 **benchmark-paper-template** 对齐骨架变更。
...
- **用法特点**：与其它七个技能**正交**。不是第 N 步，而是你走每一步时都可以随时调用的协作规则。
...
| rebuttal-drafter | 3.5 (Paper Revision LaTeX 模板) |
